...
function __ContentHighlightFeature_highlightMatchAnswers(ctx, ...args) {
    with (ctx) {
        return (function highlightMatchAnswers(questionBlock, pairs) {
        ...
        pairs.forEach((pair) => {
            ...
            if (leftNode) {
                const leftContainer = leftNode.closest('[class*="drop-sectionstyles__SDropSectionContainer"]') || leftNode;
                leftContainer.classList.add(ANSWER_HIGHLIGHT_CLASS);
                highlighted += 1;
            }

            if (rightNode) {
                const rightContainer = rightNode.closest('[class*="drag-answerstyles__SAnswer"]') || rightNode;
                rightContainer.classList.add(ANSWER_HIGHLIGHT_CLASS);
                highlighted += 1;
            }
        });
...
function __ContentHighlightFeature_applyModelAnswersHighlight(ctx, ...args) {
    with (ctx) {
        ...
        parsed.forEach((entry) => {
            ...
            if (entry.type === 'match') {
                const count = highlightMatchAnswers(questionBlock, entry.pairs);
                highlightedItems += count;
                if (entry.rawAnswer) {
