...
const TESTS_REGISTRY_STORAGE_KEY = 'god_game_tests_registry_v1';
const TESTS_FLEX_SETTINGS_STORAGE_KEY = 'god_game_tests_flex_settings_v1';
const HIGHLIGHT_COLOR_MODE_STORAGE_KEY = 'god_game_highlight_color_mode';
const HIGHLIGHT_COLOR_VALUE_STORAGE_KEY = 'god_game_highlight_color_value';
...
if (previousRuntime && typeof previousRuntime.runtimeMessageListener === 'function') {
    ...
    const scannerLogs = [];
    let journalAutoHighlightTimer = null;
    let journalAutoHighlightInFlight = false;
    let journalAutoHighlightSignature = '';
    let journalAutoHighlightLastRunAt = 0;
    ...
    function isInternetUrokJournalPage() {
        try {
            return /(^|\.)interneturok\.ru$/i.test(window.location.hostname) && window.location.pathname.startsWith('/h/journal');
        } catch (_error) {
            return false;
        }
    }

    function normalizeAutoHighlightSettings(raw) {
        const value = raw && typeof raw === 'object' ? raw : {};
        return {
            subject: String(value.subject || '').trim().toLowerCase(),
            week: String(value.week || '').trim().toLowerCase(),
            autoHighlight: Boolean(value.autoHighlight)
        };
    }

    function testMatchesFlexibleSettings(test, settings) {
        if (!test) {
            return false;
        }

        const subject = String(test.subject || '').trim().toLowerCase();
        const week = String(test.week || '').trim().toLowerCase();

        if (settings.subject && !subject.includes(settings.subject)) {
            return false;
        }

        if (settings.week && !week.includes(settings.week)) {
            return false;
        }
    ...
    function collectRegistryTestsForAutoHighlight(registry, settings) {
        const tests = [];
        const seen = new Set();
        const rawRegistry = registry && typeof registry === 'object' && !Array.isArray(registry) ? registry : {};

        Object.values(rawRegistry).forEach((entry) => {
            const entryTests = Array.isArray(entry && entry.tests) ? entry.tests : [];
            entryTests.forEach((test) => {
                if (!testMatchesFlexibleSettings(test, settings)) {
                    return;
                }
    ...
    function buildAutoHighlightSignature(settings, tests) {
        const testPart = (Array.isArray(tests) ? tests : [])
            .map((test) => `${test.itemId || ''}:${test.lessonId || ''}:${test.href || test.path || ''}:${test.questionsCount ?? ''}`)
            .join('|');
        return `${location.href}::${settings.subject}::${settings.week}::${testPart}`;
    }

    function resolveStoredHighlightColor(rawStorage) {
        const mode = String(rawStorage && rawStorage[HIGHLIGHT_COLOR_MODE_STORAGE_KEY] || '').trim().toLowerCase();
        const custom = String(rawStorage && rawStorage[HIGHLIGHT_COLOR_VALUE_STORAGE_KEY] || '').trim();
        return mode === 'custom' && custom ? custom : null;
    }

    function runJournalAutoHighlight(force = false) {
        if (!isInternetUrokJournalPage() || journalAutoHighlightInFlight) {
            return;
        }

        const now = Date.now();
        if (!force && now - journalAutoHighlightLastRunAt < 1500) {
            return;
        }

        journalAutoHighlightInFlight = true;
        journalAutoHighlightLastRunAt = now;

        chrome.storage.local.get([
            TESTS_FLEX_SETTINGS_STORAGE_KEY,
            TESTS_REGISTRY_STORAGE_KEY,
            HIGHLIGHT_COLOR_MODE_STORAGE_KEY,
            HIGHLIGHT_COLOR_VALUE_STORAGE_KEY
        ], (result) => {
            try {
                const settings = normalizeAutoHighlightSettings(result && result[TESTS_FLEX_SETTINGS_STORAGE_KEY]);
                if (!settings.autoHighlight) {
                    journalAutoHighlightSignature = '';
                    clearFoundTestsHighlight();
                    return;
                }

                const tests = collectRegistryTestsForAutoHighlight(result && result[TESTS_REGISTRY_STORAGE_KEY], settings);
                if (tests.length === 0) {
                    journalAutoHighlightSignature = '';
                    clearFoundTestsHighlight();
                    return;
                ...
                const signature = buildAutoHighlightSignature(settings, tests);
                if (!force && signature === journalAutoHighlightSignature) {
                    return;
                }

                journalAutoHighlightSignature = signature;
                const highlightColor = resolveStoredHighlightColor(result || {});
                const response = highlightFoundTests(tests, highlightColor);
                logInfo('Автоподсветка тестов из content.js', {
                    highlighted: response && response.highlighted,
                    tests: tests.length,
                    subject: settings.subject,
                    week: settings.week
                });
            } catch (error) {
                logWarn('Ошибка автоподсветки тестов из content.js', error);
            } finally {
                journalAutoHighlightInFlight = false;
            }
        });
    }

    function startJournalAutoHighlightRuntime() {
        if (!isInternetUrokJournalPage()) {
            return;
        }

        setTimeout(() => runJournalAutoHighlight(true), 700);
        setTimeout(() => runJournalAutoHighlight(true), 2200);

        if (!journalAutoHighlightTimer) {
            journalAutoHighlightTimer = setInterval(() => runJournalAutoHighlight(false), 3000);
        }
        ...
        try {
            chrome.storage.onChanged.addListener((changes, areaName) => {
                ...
                if (changes[TESTS_FLEX_SETTINGS_STORAGE_KEY]
                    || changes[TESTS_REGISTRY_STORAGE_KEY]
                    || changes[HIGHLIGHT_COLOR_MODE_STORAGE_KEY]
                    || changes[HIGHLIGHT_COLOR_VALUE_STORAGE_KEY]) {
                    journalAutoHighlightSignature = '';
                    runJournalAutoHighlight(true);
                }
            });
        } catch (_error) {
            // no-op
    ...
    const runtimeMessageListener = (request, _sender, sendResponse) => {
        try {
            ...
            if (request.action === 'findJournalTests') {
                findJournalTests(request.filters || {})
                    .then((result) => sendResponse(result))
                    .catch((error) => {
    ...
    injectPageBridge();
    loadQuestionAutoCopyState();
    startJournalAutoHighlightRuntime();
