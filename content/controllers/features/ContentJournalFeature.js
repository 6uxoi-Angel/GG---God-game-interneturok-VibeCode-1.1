...
function __ContentJournalFeature_extractQuestionsCountFromDocument(ctx, ...args) {
    with (ctx) {
        ...
        const text = normalizeText(doc && doc.body ? doc.body.textContent : htmlText);
        const match = text.match(/Количество\s+вопросов\s*:\s*(\d+)/i)
            || String(htmlText || '').match(/Количество\s*вопросов[^0-9]{0,80}(\d+)/i);
...
function __ContentJournalFeature_normalizeJournalFilterText(ctx, ...args) {
    with (ctx) {
        return (function normalizeJournalFilterText(value) {
...
function __ContentJournalFeature_filterHomeworksForFlexibleSettings(ctx, ...args) {
    with (ctx) {
        return (function filterHomeworksForFlexibleSettings(homeworks, filters = {}) {
        const normalizeFilterText = (value) => normalizeText(value)
            .toLowerCase()
            .replace(/ё/g, 'е')
            .replace(/[.,;:!?()[\]{}"'`«»]/g, ' ')
            .replace(/\s+/g, ' ')
            .trim();
        const subjectNeedle = normalizeFilterText(filters && filters.subject ? filters.subject : '');
        const weekNeedle = normalizeFilterText(filters && filters.week ? filters.week : '');
        const list = Array.isArray(homeworks) ? homeworks : [];
        if (!subjectNeedle && !weekNeedle) {
            return list;
        }

        return list.filter((item) => {
            const subjectText = normalizeFilterText(item && item.subject ? item.subject : '');
            const weekText = normalizeFilterText(item && item.week ? item.week : '');
            const subjectOk = !subjectNeedle || subjectText.includes(subjectNeedle) || subjectNeedle.includes(subjectText);
            const weekOk = !weekNeedle || weekText.includes(weekNeedle) || weekNeedle.includes(weekText);
            return subjectOk && weekOk;
        });
...
function __ContentJournalFeature_findJournalTests(ctx, ...args) {
    with (ctx) {
        return (async function findJournalTests(filters = {}) {
        const safeFilters = {
            subject: String(filters && filters.subject ? filters.subject : '').trim(),
            week: String(filters && filters.week ? filters.week : '').trim()
        };
        logInfo('Запуск поиска тестов в журнале', { url: location.href, filters: safeFilters });
        ...
        const allHomeworks = extracted.homeworks || [];
        const homeworks = __ContentJournalFeature_filterHomeworksForFlexibleSettings(ctx, allHomeworks, safeFilters);
        const skippedByMark = Number(extracted.skippedByMark || 0);
        logInfo('Из журнала извлечены ссылки ДЗ', {
            count: homeworks.length,
            totalBeforeFilter: allHomeworks.length,
            skippedByMark,
            filters: safeFilters,
        ...
        if (homeworks.length === 0) {
            if (skippedByMark === 0) {
                return {
                    success: false,
                    error: allHomeworks.length > 0 ? 'По выбранным фильтрам ДЗ не найдены' : 'Ссылки на ДЗ в журнале не найдены',
                    tests: [],
                    scanned: 0,
                    testsFound: 0,
                    failedCount: 0,
