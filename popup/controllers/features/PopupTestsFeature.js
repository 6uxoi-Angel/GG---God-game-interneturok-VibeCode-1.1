...
function __PopupTestsFeature_findJournalTestsFromProfile(ctx, ...args) {
    with (ctx) {
        ...
        getCurrentJournalPageContext((pageContext) => {
            ...
            showTestsStatus('Проверяю ссылки ДЗ в открытом журнале...', 'info', 120000);

            sendMessageToActiveTab({ action: 'findJournalTests' }, (error, response) => {
...
function __PopupTestsFeature_applyTestsHighlightOnPage(ctx, ...args) {
    with (ctx) {
        return (function applyTestsHighlightOnPage() {
        getTestsForActions((tests) => {
            if (!Array.isArray(tests) || tests.length === 0) {
                showTestsStatus('Нет найденных тестов для подсветки', 'warning');
                return;
            }

            setBusy(applyTestsHighlightBtn, true, 'Подсвечиваем...');
            const highlightColor = resolveHighlightColorForPage();
            withFreshContentScript((refreshError) => {
                if (refreshError) {
                    setBusy(applyTestsHighlightBtn, false, 'Подсветить тесты');
                    showTestsStatus(refreshError.message || 'Не удалось обновить скрипт на странице', 'error');
                    return;
                }
                ...
                sendMessageToActiveTab(
                    ...
                    (error, response) => {
                        setBusy(applyTestsHighlightBtn, false, 'Подсветить тесты');
                        if (error) {
                            showTestsStatus(error.message || 'Не удалось подсветить тесты', 'error');
                            return;
                        }

                        if (!response || !response.success) {
                            showTestsStatus((response && response.error) || 'Подсветка тестов не применена', 'warning');
                            return;
                        }

                        showTestsStatus(`Подсветка применена. Выделено: ${Number(response.highlighted || 0)}.`, 'success');
                    }
                );
            });
        });
